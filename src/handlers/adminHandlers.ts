...
class AdminHandlers {
  public addHandler = (config: Config, callback: UpdateConfigValue) => {
    return async (ctx: Context) => {
      const ChatId: string = ctx.from.id.toString();
      if (ChatId != config.SUPER_USER) {
  ...
  public removeHandler = (config: Config, callback: UpdateConfigValue) => {
    return async (ctx: Context) => {
  ...
  public showHandler = (config: Config, callback: UpdateConfigValue) => {
    return async (ctx: Context) => {
      const ChatId = ctx.from.id.toString();
      if (ChatId != config.SUPER_USER) {
        await ctx.reply(LEXICON_EN['super']);
